...
class InferenceFramework():
    ...
    def predict_entire_image(self, x):
        ...
        x_c_tensor3 = torch.from_numpy(norm_image3).float().to(device)
        x_c_tensor4 = torch.from_numpy(norm_image4).float().to(device)
        y_pred1, _ = self.model.forward(x_c_tensor1.unsqueeze(0))
        y_pred2, _ = self.model.forward(x_c_tensor2.unsqueeze(0))
        y_pred3, _ = self.model.forward(x_c_tensor3.unsqueeze(0))
        y_pred4, _ = self.model.forward(x_c_tensor4.unsqueeze(0))
        y_hat1 = (Variable(y_pred1).data).cpu().numpy()
        y_hat2 = (Variable(y_pred2).data).cpu().numpy()
        y_hat3 = (Variable(y_pred3).data).cpu().numpy()
        y_hat4 = (Variable(y_pred4).data).cpu().numpy()
    ...
    def predict_entire_image_gammas(self, x, gammas, betas):
        ...
        x_c_tensor3 = torch.from_numpy(norm_image3).float().to(device)
        x_c_tensor4 = torch.from_numpy(norm_image4).float().to(device)
        y_pred1 = self.cond_fusionnet_gn_fun(x_c_tensor1.unsqueeze(0), gammas, betas)
        y_pred2 = self.cond_fusionnet_gn_fun(x_c_tensor2.unsqueeze(0), gammas, betas)
        y_pred3 = self.cond_fusionnet_gn_fun(x_c_tensor3.unsqueeze(0), gammas, betas)
        y_pred4 = self.cond_fusionnet_gn_fun(x_c_tensor4.unsqueeze(0), gammas, betas)
        y_hat1 = (Variable(y_pred1).data).cpu().numpy()
        y_hat2 = (Variable(y_pred2).data).cpu().numpy()
        y_hat3 = (Variable(y_pred3).data).cpu().numpy()
        y_hat4 = (Variable(y_pred4).data).cpu().numpy()
